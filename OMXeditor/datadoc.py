...
class DataDoc:
    ...
    def __init__(self, MRC_path):
        ...
        # handy.
        self.size = numpy.array([self.numWavelengths, numTimepoints, numZ, numY, numX],
                                dtype = numpy.int)
    ...
    def takeProjectedSlice(self, axes, projectionAxis, shouldTransform,
                           order = 1):
        """As takeSlice, but do a max-intensity projection across one axis.
        This becomes impossible to do efficiently if we have rotation or
        scaling in a given wavelength, so we just have to transform the entire
        volume. It gets *really* expensive if we want to do projections across
        time with this...

        TODO:
            * We could be a bit more efficient here since only the wavelengths
    ...
    def mapCoords(self, data, targetCoords, targetShape, axes, order):
        """Inverse-transform the provided coordinates and use them to look up
        into the given array, to generate a transformed slice of the specified
        shape along the specified axes.

        Args:
            data - A 5D array of pixel data (WTZYX)
            targetCoords - 4D array of WXYZ coordinates.
            targetShape - Shape of the resulting slice.
            axes - Axes the slice cuts along.
            order - Spline order to use when mapping. Lower is faster but
                less accurate
        """
    ...
    def getValuesAt(self, coord):
        """Return the value for each wavelength at the specified TZYX
        coordinate, taking transforms into account. Also return the
        transformed coordinates.

        TODO:
            This copies a fair amount of logic from self.mapCoords.
        """
        transforms = self.getTransformationMatrices()
        inverseTransforms = [numpy.linalg.inv(matrix) for matrix in transforms]
    ...
    def transformArray(self, inData, dx, dy, dz, angle, zoom, order = 3):
        """Apply a transformation to an input 3D array in ZYX order. Angle
        rotates each slice, zoom scales each slice (i.e. neither is 3D).
        """
    ...
    def registerAlignmentCallback(self, callback):
        """Register a callback to be invoked when the alignment parameters
        change.
        """
    ...
    def saveSelection(self, wavelengths = [], timepoints = [],
            savePath = None):
        """subset of alignAndCrop - without alignment/cropping...
        ...
        TODO
            * Refactor alignAndCrop so it uses this (duplicate) code.
            * Optionally create & return new DataDoc object(s)?
        """
        if not wavelengths:
    ...
    def getExtendedHeaderIndex(self, timepoint, wavelength, zIndex):
        """Return the number of the section for the extended header, based
        on the provided indices and the order in which data is stored, as
        indicated by the ImgSequence parameter:
        # 0: ztw
        # 1: wzt
        # 2: zwt
        """
    ...
    def getSliceSize(self, axis1, axis2):
        """Get the size of a slice in the specified dimensions. Dimensions
        are as ordered in self.size
        """
    ...
    def getSliceCoords(self, axes = None):
        ...
        Args:
            axes - A list of axes, e.g. [0, 3] for (time, Y). If None, then
                operate on all axes.
        """
        if axes is None:
            axes = range(5)
    ...
    def moveSliceLines(self, offset):
        """Move self.curViewIndex by the specified amount, ensuring that
        we stay in-bounds.
        """
